...
to all the functions, while maintaining the parens in a pretty state."
  ...
  (let (bnd)
    (lispy-dotimes N
      (cond ((and (> (minibuffer-depth) 0) (lispy-right-p))
             (exit-minibuffer))
            ((looking-back ":")
             (backward-char 1)
            ...
            ((lispy-left-p)
             (lispy-newline-and-indent-plain))
            ((lispy-right-p) (lispy-newline-and-indent-plain))
            ((when (setq bnd (lispy--bounds-list))
               (goto-char (cdr bnd))))
            ((looking-at lispy-right)
             (lispy-right 1))
            ((lispy-looking-back "^ +")
             (if (re-search-forward lispy-right (line-end-position) t)
                 (backward-char 1)
               (move-end-of-line 1)))
            (t (lispy-newline-and-indent-plain)
               ;; (when bnd
               ;;   (goto-char (cdr bnd)))
